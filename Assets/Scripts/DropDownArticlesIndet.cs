...
public class DropDownArticlesIndet : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        {
            if (i > GameManager.totalArticulosInd.Count - 1)
                buttons[i].gameObject.SetActive(false);
            else
            {
                if (SingletonLenguage.GetInstance().GetLenguage() == SingletonLenguage.Lenguage.CASTELLANO && GameManager.totalArticulosInd[i].articuloSpanish == "")
                {
                    buttons[i].gameObject.SetActive(false);
                }
                else if (SingletonLenguage.GetInstance().GetLenguage() == SingletonLenguage.Lenguage.CATALAN && GameManager.totalArticulosInd[i].articuloCatalan == "")
                {
                    buttons[i].gameObject.SetActive(false);
                }
                else
                {
                    ...
                    buttons[i].SetArticle(GameManager.totalArticulosInd[i]);
                }
            }
