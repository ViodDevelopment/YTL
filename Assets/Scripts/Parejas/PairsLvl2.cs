...
public class PairsLvl2 : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        originalSizeText = texto.gameObject.transform.localScale.x;
        ...
        if (managerOnlyOne != null)
        {
            ...
            if (m_PieceClicked && (Input.GetMouseButtonUp(0) || (Input.touchCount > 0 && Input.GetTouch(0).phase == TouchPhase.Ended)) && timer == 0)
            {
                ...
                if (dentro)
                {
                    ...
                    texto.gameObject.transform.localScale = Vector3.one * originalSizeText;
                    colision.gameObject.GetComponentInChildren<Text>().gameObject.transform.localScale = Vector3.one * originalSizeText;
